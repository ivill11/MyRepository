let content = (function () {
    ...
    //  author: 'Родион Власов',
    //  image: 'kek https://cdn.tribuna.com/fetch/?url=http%3A%2F%2Ftu9srvbirvvtmir3cg1lzglhlnrozxbyb3zpbmnllmnvbq00.g00.theprovince.com%2Fg00%2F2_dGhlcHJvdmluY2UuY29t_%2FTU9SRVBIRVVTMiRodHRwOi8vd3BtZWRpYS50aGVwcm92aW5jZS5jb20vMjAxNy8wMS9yZWNvcmRpbmctMS5naWY%252Fdz04NzImaD00OTEmaTEwYy5tYXJrLmltYWdlLnR5cGU%253D_%24%2F%24%2F%24%2F%24',
    ...
    function editArticle(id, article) {
        ...
        let newArticle = Object.assign(rawArticle, article);
...
let dom = (function () {
    ...
    function fillTemplate(article) {
        let copy = template.querySelector('.news').cloneNode(true);
        copy.id = article.id;
        if (article.image)
    ...
    function editArtice(id, atricle) {
        let sucess = content.editArticle(id, article);
        if (sucess) {
            let found = document.getElementById(id);
...
function authorize(user) {
    if (user) {
        ...
        for (let btn of document.querySelectorAll('.edit')) {
...
function logIn() {
    let username = document.getElementById("autinput").value;
    let password = document.getElementById("autinput1").value;
    userModel.login({username, password}).then(
        name => window.location = 'index.html',
        error => {
            document.querySelector('.maintape').style.display = 'none';
            document.querySelector('.add-news').style.display = 'none';
            document.querySelector('.authorization').style.display = 'none';
...
function showMoreArticles() {
    let articles = content.getArticles(0, curNews, currentFilter);
...
function refresh() {
    let articles = content.getArticles(0, curNews - 3, currentFilter);
    document.querySelector('.tape').innerHTML = '';
    dom.display(articles);
}
...
function eventHandler(event) {
    ...
    let article = content.getArticle(post.id);
    ...
    if (event.target.classList.contains('btn')) {
        ...
        document.querySelector('.authorization').style.display = 'none';
        document.querySelector('.error').style.display = 'none';
        let detailed = document.querySelector('.newspage');
...
document.addEventListener('DOMContentLoaded', () => {
    httpModel.getAllArticles().then(
        articles => {
            content.fillArray(articles);
            showMoreArticles();
            userModel.getUsername().then(name => authorize(name), error => authorize());
        }
    );
});